    ...
    <UICard.Root class="w-sm">
      <UICard.Header>
        ...
        <UICard.Subtitle>
          ...
          <div v-else class="h-[24px] flex items-center gap-3">
      ...
      <UICard.Body>
        <NuxtImg
          ...
          fit="center"
          height="200px"
          class="h-[200px] w-full rounded-moon-s-sm object-cover"
        />
        <UISkeleton v-else class="h-[200px] w-full rounded-moon-s-sm!" />
      </UICard.Body>
      <UICard.Footer class="flex justify-between">
        <span class="text-nowrap text-moon-32 font-bold">
          <template v-if="fin">
            $ 169
          </template>
          <UISkeleton v-else class="h-[40px] w-18" />
        </span>
        <UIButton icon="i-lucide:shopping-basket" icon-pos="right" @click="fin = !fin">
          Add to cart
        </UIButton>
      </UICard.Footer>
    </UICard.Root>
